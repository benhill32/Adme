...
function getdata(tx) {
    ...
    var sql = "select MAD.ID as ID,MAD.StartDate as StartDate ,MAD.EndDate as EndDate ,MAD.ItemName as ItemName,MAD.Details as Details ,MAD.Price as Price ,MAD.URL as URL, MBN.Icon as Icon,MAD.DeletedateUTC as DeletedateUTC, MBC.Follow as Follow " +
        ...
        "WHERE  datetime(MAD.EndDate) >=  datetime('" + year + "-" + month + "-" + day + " " + hours + ":" + mins + ":00') and MAD.DeletedateUTC = 'null' and MBN.DeletedateUTC = 'null' ";
     //alert(sql);
...
function getdata_success(tx, results) {
    ...
    for (var i=0; i<len; i++) {
       ...
       var target_date = new Date(day2 + "/" + month[month2] + "/" + year2 + " " + h).getTime();
        ...
        '<div align="center" class="dailydivbottom"  >' + menu.Price + " " + menu.ItemName + " - " + menu.Follow +
        '</div>' +
        '</Div>');
